...
- What does the sample code generated by the wizard do? Explain what
  ...
  **appengine-web.xml** and **index.jsp** files in a few sentences.
  ...
  The class `HelloAppEngine` is a Servlet that extends the `HttpServlet` class. It overrides the `doGet` method to handle the HTTP GET requests. The `doGet` method writes a response to the client with the message "Hello App Engine!".
  ...
  The `web.xml` file is a deployment descriptor for the web application. The `welcome-file-list` is used to specify the file that should be loaded when the user visits the application by default. It defines everything the server needs to know concerning the servlets, listeners, initalization parameters, etc...
  ...
  index.jsp
   ...
   It is the default page that is loaded when the user visits the application. It is a JSP file that contains HTML and Java code. In this case, it contains a simple HTML page with a title and a message "Hello App Engine!".
